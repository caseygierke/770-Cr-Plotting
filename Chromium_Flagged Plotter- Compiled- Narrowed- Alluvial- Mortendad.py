# Chromium_Flagged Plotter- Compiled- Narrowed- Alluvial- Mortendad.py

# Chromium_Flagged Plotter- Appendix Table.py
...
input = 'Compiled- Narrowed- Alluvial- Mortendad'
inputFolder = 'Inputs'

# Make dictionary for appendix figures
figureDict = {}
Dictin = open(path+os.sep+'Python'+os.sep+'Plotting'+os.sep+inputFolder+os.sep+input+'.txt','r')
...
locID_index = headers.index('Location I')
lat_index = headers.index('Latitude')
long_index = headers.index('Longitude')
type_index = headers.index('Aquifer')
fig_index = headers.index('Figure Number')

# Read in data
for line in Dictin:
	Columns = line.split('\t')
	figureDict[Columns[0]] = {'Lat': Columns[lat_index], 'Long': Columns[long_index], 'Aquifer': Columns[type_index], 'Figure Number': Columns[fig_index]}
	
# # Short circuit
# figureDict = {'SIMR-2': figureDict['SIMR-2']}

# Create an outfile for storing maxes
fout = open(path+os.sep+'Python'+os.sep+'Plotting'+os.sep+'Outputs'+os.sep+'Maxes- '+input+'.txt','w')

# Create an outfile for storing maxes
foutExceedance = open(path+os.sep+'Python'+os.sep+'Plotting'+os.sep+'Outputs'+os.sep+'Exceedance- '+input+'.txt','w')

# Create an outfile for defining active locations
foutActive = open(path+os.sep+'Python'+os.sep+'Plotting'+os.sep+'Outputs'+os.sep+'Active- '+input+'.txt','w')
...
for location in figureDict:
	...
	# Save the figure
	fig.savefig(path+os.sep+'Python'+os.sep+'Plotting'+os.sep+'Outputs'+os.sep+'Figures'+os.sep+input+os.sep+figureDict[location]['Aquifer']+'- '+figureDict[location]['Figure Number']+'- '+location+'.png',dpi=500)
